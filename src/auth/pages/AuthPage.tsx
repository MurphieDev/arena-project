...
function BrandingPanel() {
  ...
  useEffect(() => {
    ...
    getDocs(
      firestoreQuery(collection(db, 'users'), where('role', '==', 'tipster'), limit(10))
    ).then(snap => {
      if (snap.size > 0) {
        const tipsters = snap.docs
          .map(d => ({ id: d.id, ...d.data() }))
          .sort((a: any, b: any) => (b.winRate || 0) - (a.winRate || 0))
          .slice(0, 3)
          .map((d: any) => {
          const sport = d.sports?.[0] === 'Football' ? '⚽' : d.sports?.[0] === 'Basketball' ? '🏀' : d.sports?.[0] === 'F1' ? '🏎️' : '🎯';
          return {
            name: d.displayName || 'Tipster',
