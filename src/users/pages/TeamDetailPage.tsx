...
async function fetchTeamData(teamId: number) {
  try {
    const [teamRes, fixturesRes] = await Promise.all([
      fetch(`https://v3.football.api-sports.io/teams?id=${teamId}`, { headers: { 'x-apisports-key': API_KEY } }),
      fetch(`https://v3.football.api-sports.io/fixtures?team=${teamId}&season=2025&last=5`, { headers: { 'x-apisports-key': API_KEY } }),
    ]);
    const [teamData, fixtureData] = await Promise.all([teamRes.json(), fixturesRes.json()]);
    return {
      team: teamData.response?.[0]?.team || null,
      fixtures: fixtureData.response || [],
    };
  } catch { return { team: null, fixtures: [] }; }
}
...
export function TeamDetailPage({ onBack }: TeamDetailPageProps) {
  ...
  return (
    <div>
      {/* Header */}
      <div className="flex items-center gap-3 px-4 py-3 border-b border-[#1f1f1f] sticky top-0 z-20 bg-black/90 backdrop-blur-md">
